...
export default function Update() {
	...
	async function submitForm(e) {
		...
		if (file) {
			...
			await http.put(`${process.env.NEXT_PUBLIC_DOMAIN}/blog/categories/update/${id}`,formData)
			.then((res)=>{
			...
			await http.put(`${process.env.NEXT_PUBLIC_DOMAIN}/blog/categories/update/${id}`,{name, desc, parentId})
			.then((res)=>{
			notify("success", "successfully Added!");
				console.log(res.data);
			router.push('/admin/blog/category');
				...
				<div className="card-body">
					...
					<div className="form-group">
						...
						<div className="ql-wrapper ql-wrapper-demo bg-light">
                            <textarea style={{height: '200px'}} onChange={(e)=>setDesc(e.target.value)} className="form-control" id="" name="desc">{desc}</textarea>
						</div>
					</div>
