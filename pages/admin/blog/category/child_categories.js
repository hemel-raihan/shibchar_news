...
export default function ChildCategories({item,subCat}) {
  ...
  //const {data, loading, error} = useFetch(`http://localhost:5000/api/blog/categories/${item.parentId}`)
  return (
    <>
    {item?.childs?.map((child, i)=>(
     ...
     {/* <ul className="transfer-double-group-list-li-ul transfer-double-group-list-li-ul-16368784927512">
        <li className="transfer-double-group-list-li-ul-li transfer-double-group-list-li-ul-li-16368784927512">
          <div className="checkbox-group">
                <input type="checkbox" name="parent_id" value={child._id} onChange={(e)=>setChildId(e.target.value)} className="checkbox-normal group-checkbox-item-1636878492751 belongs-group-0-1636878492751" id={`group_${item._id}_checkbox_${i}_1636878492751`} />
                <label htmlFor={`group_${item._id}_checkbox_${i}_1636878492751`} className="group-checkbox-name-1636878492751">{child.name}</label>
          </div>
